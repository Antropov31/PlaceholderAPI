...
public final class CharsReplacer implements Replacer {
  ...
  public String apply(@NotNull final String text, @Nullable final OfflinePlayer player,
    ...
    loop: do {
      ...
      if (expansion != null) {
        // Isolate the expansion: a single broken/misbehaving expansion (for example a
        // Vault expansion that failed to load) must never be able to throw its way up
        // into the caller and the downstream text pipeline (Adventure), which would
        // corrupt the render and kick the player. Catch everything, log it once, and
        // fall back to leaving the placeholder untouched.
        try {
          replacement = expansion.onRequest(player, parameters);
        } catch (final Throwable throwable) {
          Msg.severe("Could not parse placeholder %s%s_%s%s from expansion %s", throwable,
              head, identifier, parameters, tail, expansion.getIdentifier());
          replacement = null;
        }
      }
